/**
 * Центральный набор значений по умолчанию конфигурации Pielet.
 * Значения не должны дублироваться нигде в коде.
 * Визуальные значения по умолчанию живут исключительно в CSS.
 *
 * @typedef {import('../types.js').PieletConfig} PieletConfig
 */

/** @type {PieletConfig} */
export const DEFAULT_CONFIG = {
    size: 240,
    centerSize: 72,
    gap: 4,

    startAngle: -90,
    direction: 'clockwise',

    interactionMode: 'click',

    button: 'left',
    closeDistance: 48,

    fit: 'circle',

    // Выравнивание шрифта text-пунктов (только при fit: 'square')
    unifyText: false,

    // Задержка (мс) открытия сабменю при наведении в hold-режиме.
    // 0 отключает hover-открытие (остаётся только click-пайплайн).
    submenuDelay: 400,

    // Индикация пунктов-сабменю: 'arc' (дуга у внутреннего радиуса),
    // 'chevron' (стрелка у внешнего края) или 'both'.
    // Отдельного варианта «без индикации» нет — сабменю всегда помечается.
    submenuIndicator: 'both'
};