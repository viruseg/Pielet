 ...
 * Шеврон «сидит» на внешнем крае сектора: центр — на 0.25·size за кольцом,
 * внутренняя кромка на 0.25·size входит внутрь кольца, внешняя — торчит
 * за него на 0.75·size. Положение от размера меню не зависит, стрелка всегда
 * указывает радиально наружу и остаётся радиально дальше контента (контент
 * при любом fit центрируется внутри кольца, на contentRadius ≤ meanRadius).
 * @type {number}
 */
const SUBMENU_CHEVRON_EXTERNAL_OFFSET_RATIO = 0.25;
 ...
 * центр — за внешним краем кольца на луче mid (на 0.25·size дальше
 * внешнего радиуса). Положение стабильно при любом размере меню и не зависит
 * от контента пункта: шеврон всегда радиально дальше контента и указывает
 * наружу — не сливается с ним ни в circle-, ни в square-fit.
 *
