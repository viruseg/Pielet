...
| `action` | функция `(id, menu, coords) => {}` (опционально) | Вызывается при выборе. Первый аргумент — `id` пункта, второй — экземпляр меню, третий — координаты указателя в момент клика по пункту. |
