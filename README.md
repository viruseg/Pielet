...
| `unifyText` | `false` | Выровнять шрифт всех `text`-пунктов по наименьшему влезающему размеру (шрифт самого длинного текста). Действует только при `fit: 'square'`, в `'circle'` игнорируется. |
