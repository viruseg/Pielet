...
- Пункт-сабменю автоматически помечается индикатором (по умолчанию дуга у основания + стрелка за внешним краем кольца). Тип задаётся `submenuIndicator: 'arc' | 'chevron' | 'both'`, цвет — CSS-переменной `--pielet-submenu-indicator`.