...
Порядок: `select`-событие (`detail.id` — строковый id пункта, `detail.menu` — экземпляр меню, `detail.coords` — координаты клика) → мгновенное удаление DOM и слушателей → `action(id, menu, coords)` — тот же id первым аргументом, экземпляр меню вторым и координаты клика третьим. Исключения не подавляются. Это позволяет action открывать новое меню без конфликтов с закрывающимся.
